...
void test(void *pvParameters)
{
    ...
    while (1)
    {
        ESP_ERROR_CHECK(led_strip_fill(&strip, 0, strip.length, colors[node_level]));
        ESP_ERROR_CHECK(led_strip_flush(&strip));
        printf("Helo World from the LED task and the node level is : %d ", node_level);
