...
void led(void *pvParameters)
{
  ...
  while(1){
    while (mwifi_is_connected())
    {
        ...
        //Still add buzzer
          ESP_ERROR_CHECK(led_strip_fill(&strip, 0, 1, colors[2]));
          ESP_ERROR_CHECK(led_strip_flush(&strip));
     ...
     /*
        ...
        if (system_state== 1){  //Loaud red and orange lights
            for (size_t i = 0; i < 20; i++)
            {
             printf(" -ForegroundColor Green -NoNewline Hello ?? "); 
    ESP_ERROR_CHECK(led_strip_fill(&strip, 0, 1, colors[2]));
    ESP_ERROR_CHECK(led_strip_flush(&strip));
    vTaskDelay(200 / portTICK_PERIOD_MS);
     ESP_ERROR_CHECK(led_strip_fill(&strip, 0, strip.length, colors[0]));
    ...
    vTaskDelay(200 / portTICK_PERIOD_MS);
        ...
        }else if (system_state == 2){   //Silent blue and green lights 
           
        }else if(system_state == 3){        //3 is switching off the lights / alarm 
             ESP_ERROR_CHECK(led_strip_fill(&strip, 0, strip.length, colors[0]));
             ESP_ERROR_CHECK(led_strip_flush(&strip));
             vTaskDelay(200 / portTICK_PERIOD_MS);
        }else{
             printf("System state is not 1 or 2 , it is  %d ", system_state );
        }
    */ 
    ESP_ERROR_CHECK(led_strip_fill(&strip, 0, 1, colors[0]));
    ESP_ERROR_CHECK(led_strip_flush(&strip));
...
/*
ESP_ERROR_CHECK(led_strip_fill(&strip, 0, 1, colors[1]));
ESP_ERROR_CHECK(led_strip_flush(&strip));
vTaskDelay(750 / portTICK_PERIOD_MS);
ESP_ERROR_CHECK(led_strip_fill(&strip, 0, strip.length, colors[0]));
ESP_ERROR_CHECK(led_strip_flush(&strip));
